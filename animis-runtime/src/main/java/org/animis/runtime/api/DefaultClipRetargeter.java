...
import org.animis.clip.CompressedTrackData;
import org.animis.clip.TransformTrack;
import org.animis.retarget.JointMapping;
...
public final class DefaultClipRetargeter implements ClipRetargeter {
  ...
  public Clip retarget(
    ...
    final Optional<org.animis.clip.RootMotionDef> rootMotion = source.rootMotion().flatMap(def -> {
      final Joint root = sourceSkeleton.joints().get(def.rootJoint());
      final Integer mappedRoot = targetByName.get(root.name());
      if (mappedRoot == null) {
        return Optional.empty();
      }
      return Optional.of(new org.animis.clip.RootMotionDef(
          mappedRoot,
