...
import dev.ljramones.animis.retarget.JointMapping;
...
public final class DefaultClipRetargeter implements ClipRetargeter {
  ...
  public Clip retarget(
    ...
    final Optional<dev.ljramones.animis.clip.RootMotionDef> rootMotion = source.rootMotion().flatMap(def -> {
      final Joint root = sourceSkeleton.joints().get(def.rootJoint());
      final Integer mappedRoot = targetByName.get(root.name());
      if (mappedRoot == null) {
        return Optional.empty();
      }
      return Optional.of(new dev.ljramones.animis.clip.RootMotionDef(
          mappedRoot,
